...
async function infiniteHandler($state) {
  ...
  try {
    ...
    // 다음 페이지 계산 (fallback 포함)
    const backendHasMore = (typeof res.data?.hasMore === 'boolean')
      ? res.data.hasMore
      : list.length === size.value; // ← fallback

    if (backendHasMore) {
      page.value = typeof res.data?.nextPage === 'number' ? res.data.nextPage : (page.value + 1);
      $state.loaded();
    } else {
      hasMore.value = false;
      $state.complete();
