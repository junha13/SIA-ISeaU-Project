...
const { execute: verifyUserForPasswordReset } = useApi('post', '/auth/find-userNumber');

// (선택) SMS 개발용 엔드포인트 - 프론트에서 참조 중이므로 정의
const { execute: sendSms } = useApi('post', '/auth/send-sms');
const { execute: verifySms } = useApi('post', '/auth/verify-sms');
